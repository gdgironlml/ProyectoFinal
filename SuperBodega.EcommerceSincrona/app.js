// Para probar con locarlhost
// const API_BASE = 'http://localhost:8080/api';
const API_BASE = 'https://refill-blurt-utter.ngrok-free.dev/api';

const CLIENT_STORAGE_KEY = 'superbodega.ecommerce.sync.client';
...
async function loadTienda(page = 1) {
    ...
    try {
        ...
        if (!productos || productos.length === 0) {
            setStatus('tiendaStatus', 'Sin productos en esta categoria o pagina.');
            ...
            <div class="col-md-6 col-xl-4">
                <article class="card product-card h-100">
                    <div class="card-body d-flex flex-column">
                        ...
                        <p class="text-muted small mb-1">${escapeHtml(p.descripcion || 'Sin descripcion')}</p>
        ...
        if (data.totalPages) {
            ...
            setStatus('tiendaStatus', `Mostrando pagina ${currentPage} de ${data.totalPages} (Total: ${data.totalItems} productos)`);
        } else {
...
async function loadCarrito() {
    ...
    try {
        ...
        document.getElementById('btnProcesarVentaSync').disabled = false;

        body.querySelectorAll('.btn-eliminar-item').forEach((button) => {
            button.onclick = async () => {
                await apiFetch(`/Carritos/${clienteId}/items/${button.dataset.id}`, { method: 'DELETE' }, 'quitar item');
                await loadCarrito();
...
function setSyncProcessingState(isProcessing) {
    const syncButton = document.getElementById('btnProcesarVentaSync');
    const spinner = document.getElementById('syncSpinner');
    const text = document.getElementById('syncButtonText');
    const closeHeader = document.getElementById('btnCerrarModal');
    const closeFooter = document.getElementById('btnCerrarModalFooter');
    ...
    if (syncButton) syncButton.disabled = isProcessing;
    if (spinner) spinner.classList.toggle('d-none', !isProcessing);
    if (text) text.textContent = isProcessing ? 'Procesando...' : 'Pagar sincrono';
    if (closeHeader) closeHeader.disabled = isProcessing;
    if (closeFooter) closeFooter.disabled = isProcessing;
    if (cartButton) cartButton.disabled = isProcessing;
    if (refreshButton) refreshButton.disabled = isProcessing;
    if (searchButton) searchButton.disabled = isProcessing;
}

function setGlobalSyncState(isBusy) {
...
async function procesarVentaCarritoSync() {
    const client = await ensureCurrentClient();
    if (!client) return;

    const endpoint = `/Carritos/${client.id}/checkout/sync`;

    try {
        setSyncProcessingState(true);
        await apiFetch(endpoint, { method: 'POST' }, 'confirmar compra');
        showAlert('Compra procesada de forma sincrona.', 'success');
        if (carritoModal) {
            carritoModal.hide();
        }
        await loadTienda(currentPage);
...
async function onTiendaClick(event) {
    ...
    try {
        ...
        showAlert('Producto agregado al carrito con exito.', 'success');
        await loadTienda(currentPage);
        await loadCartCount();
    } catch (error) {
        showAlert(error.message, 'danger');
