...
$newmanPath = Join-Path $reports 'newman-report.json'
$k6Path = Join-Path $reports 'k6-summary.json'

if (-not (Test-Path $newmanPath)) {
	throw "No se encontro el reporte de Newman en $newmanPath"
}

if (-not (Test-Path $k6Path)) {
	throw "No se encontro el resumen de k6 en $k6Path"
}

$newman = Get-Content $newmanPath -Raw | ConvertFrom-Json
...
$newmanDetails = if ($newmanFailedResponseCount -gt 0) {
	...
	'Detalles observados: ninguna peticion con estado no 2xx.'
}
...
- Peticiones fallidas segun el reporte: $newmanFailedRequests
...
- Duracion aproximada de la ejecucion: $newmanElapsedSec s
...
$(if ($newmanObservedFailures -eq 0 -and $k6Failed -eq 0) { 'Las pruebas funcionaron correctamente y no se detectaron fallos en los casos de uso ni en la carga.' } else { 'Hubo observaciones o fallos en alguna de las pruebas, asi que conviene revisar los detalles tecnicos de los reportes JSON.' })
"@

$summaryPath = Join-Path $reports 'resumen-pruebas.md'
$summary | Set-Content -Path $summaryPath -Encoding UTF8
Write-Host "Resumen generado en $summaryPath"